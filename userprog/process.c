...
//  * thread id, or TID_ERROR if the thread cannot be created.
//  * Notice that THIS SHOULD BE CALLED ONCE. */
// tid_t process_create_initd(const char *f_name)
// {
// 	char *file_name = f_name;
// 	char *fn_copy;
// 	tid_t tid;
...
// 	_if.ds = _if.es = _if.ss = SEL_UDSEG; // data
// 	_if.cs = SEL_UCSEG;					  // code
// 	_if.eflags = FLAG_IF | FLAG_MBS;
// 	success = load(file_name, &_if);
// 	if (!success)
// 	{
...
// 	/* And then load the binary */
// 	success = load(file_name, &_if); // return true or false
...
// 		argv[ctn++] = token;
// 	}
// 	argument_stack(argv, ctn, &if_->rsp);
